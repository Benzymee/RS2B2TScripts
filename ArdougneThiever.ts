...
/** East Ardougne bank (food restock). */
const BANK_STAND = new Tile(2655, 3286, 0);

/** Coins / gp — RS2 item id 995. */
const COINS_ID = 995;
const COINS_NAME = 'Coins';

/** Mule partner — receive all Coins at East Ardougne south bank (GoldHost stand). */
const MULE_NAME = 'Ben';
const MULE_STAND = new Tile(2653, 3284, 0);
const MULE_TRADE_RANGE = 2;
/** Stay inside East Ardougne bank while waiting / trading. */
const BANK_MULE_LEASH = 8;
/** Banked Coins (id 995) must exceed this before a Ben handoff. */
const MULE_GP_THRESHOLD = 100_000;
...
/**
 * Auto pickpocket target from current Thieving level.
 * 1–24 Men/Women, 25–39 Warrior women, 40–54 Ardougne Guards, 55–79 Knights, 80+ Heroes.
 * Paladin is manual only.
 */
...
/**
 * Exact in-game names. Cake uses leftovers first: Slice of cake → 2/3 cake → Cake.
 */
...
/** Dropdown food families: eat order vs bank withdraw order. */
const FOOD_TYPES = {
    Cake: {
...
/** Catherby catch-and-cook restock (Cage+Harpoon → Range → bank). */
...
/** East Ardougne Baker stall restock when Food is Cake. */
...
function carriedCakes() {
    return Inventory.items()
        .filter(i => isCakeFoodName(i.name))
        .reduce((n, i) => n + Math.max(0, i.count), 0);
}
...
/** Indoor targets (Men/Women houses, Paladin castle) — keep doors open; don't escape outside. */
...
class ArdougneThiever extends LoopingBotBase {
    ...
    override async onStart(): Promise<void> {
        ...
        this.log(
            `Benzyme's Ardougne Thiever — bank only if food < withdraw amount, then ` +
                (this.targetPref === TARGET_AUTO ? `auto (Thieving ${have}) ` : '') +
                ...
                `; steal cakes ${this.stealCakesWanted() ? 'on' : 'off'}` +
                (this.stealCakesWanted()
                    ? ` (${CAKE_STALL_GOAL} from Baker stall when bank has no cake)`
                    : '') +
                `; catch & cook ${this.catchAndCookWanted() ? 'on' : 'off'}` +
                (this.catchAndCookWanted()
                    ? ` (${CATCH_COOK_GOAL} lobsters at Catherby when bank has no Lobster)`
                    : this.stealCakesWanted()
                      ? ''
                      : ' (will stop if bank food is gone)') +
                (this.muleWanted()
                    ? `; mule when bank Coins (id ${COINS_ID}) > ${fmtGp(MULE_GP_THRESHOLD)} → withdraw all → ${MULE_NAME} @ ${MULE_STAND.x},${MULE_STAND.z}`
                    : '')
        );
    ...
    override onStop(): void {
        ...
        this.log(
            `stopped — ${this.steals} steals, ${this.fails} fails, ${this.eats} eats, ` +
                ...
                `muled ${fmtGp(this.muled)}gp (${this.muleTrips} trips), ` +
                `catch & cook ${this.catchCookTrips} trips (${this.catchCookCooked} cooked), ` +
                `steal cakes ${this.stealCakesTrips} trips (${this.stealCakesBanked} cakes), ` +
    ...
    syncPrefs(opts: any = {}) {
        ...
        this.catchAndCook = readPrefBool(
            'catchAndCook',
            typeof this.settings?.bool === 'function'
                ? this.settings.bool('catchAndCook', true)
                : true
        );
        this.stealCakes = readPrefBool(
            'stealCakes',
            typeof this.settings?.bool === 'function'
                ? this.settings.bool('stealCakes', true)
                : true
        );
    ...
    bankedLobsterCount() {
        ...
        const items = typeof Bank.items === 'function' ? Bank.items() ?? [] : [];
        return items
            .filter(i => isCookedLobster(i.name))
            .reduce((n, i) => n + Math.max(0, i.count), 0);
    }
    ...
    resolveWithdrawPlan(amount) {
        ...
        if (plan.length > 0) {
            if (this.foodType === 'Cake' && plan[0].name !== FOOD_CAKE) {
                ...
                this.foodType === FOOD_TYPE_SHRIMP &&
                !nameEq(plan[0].name, FOOD_SHRIMP) &&
                !nameEq(plan[0].name, FOOD_SHRIMP_ALT)
            ) {
                this.log(
                    `no ${FOOD_SHRIMP} in bank — withdrawing ${plan.map(p => `${p.take}× ${p.name}`).join(', ')}`
                );
            } else if (
                this.foodType !== FOOD_LOBSTER &&
                nameEq(plan[0].name, FOOD_LOBSTER)
            ) {
                this.log(
                    `no ${this.foodType} in bank — withdrawing ${plan.map(p => `${p.take}× ${p.name}`).join(', ')} ` +
                        `(${this.bankedLobsterCount()} Lobster in bank)`
                );
            }
            return plan;
        }
    ...
    async tryCatchCookOrStop(context) {
        ...
        if (fish < CATCH_COOK_FISH_LVL || cook < CATCH_COOK_COOK_LVL) {
            this.log(
                `${context}: WARNING catch & cook needs Fishing ${CATCH_COOK_FISH_LVL} and Cooking ${CATCH_COOK_COOK_LVL} ` +
                    `(have ${fish}/${cook}) — going to Catherby anyway`
            );
        }
        this.beginCatchCook(context);
        await this.catchCookTick();
    ...
    override async loop(): Promise<void> {
        ...
        // Outdoor targets only: if trapped in a house, open the door and get back outside.
        // Man / Woman / Paladin are indoors — escaping would cancel pickpocketing.
        ...
        if (needsHouseDoors(cfg)) {
            if (npc.distance() > 1) {
                // Indoor NPC may be behind a door — open it if they aren't in melee.
                await this.clearDoorsToward(npc.tile());
            } else if (this.findAdjacentShutDoor(2)) {
                // Paladin/Man rooms: door can slam while we're already in melee.
    ...
    findTarget() {
        ...
        let q = Npcs.query()
            ...
            .within(cfg.leash + 4)
            .where(n => names.includes((n.name ?? '').toLowerCase()))
            .where(n => !n.inCombat);
        if (cfg.npcId != null) {
    ...
    async bankFoodRestock() {
        ...
        if (!Bank.isOpen()) {
            this.log(
                `out of food — banking, withdraw up to ${this.foodWithdraw}× ${prefer}${extraFood}`
            );
            if (
                !(await Banking.open({
                    stand: BANK_STAND,
    ...
    async clearDoorsForMan(anchor) {
        ...
        for (let i = 0; i < 4; i++) {
            const door = Locs.query()
                .where(l => isShutDoor(l))
                .where(l => Tile.from(l.tile()).distanceTo(focus) <= radius)
                .nearest();
            if (!door) {
                // No door on anchor — still clear whatever is next to us.
    ...
    beginStealCakes(context) {
        ...
        this.log(
            `${context}: no cake in bank — steal ${CAKE_STALL_GOAL} from Baker stall ` +
                `@ ${CAKE_STALL_STAND.x},${CAKE_STALL_STAND.z}, then resume thieving`
        );
    ...
    findBakerStall() {
        return (
            Locs.query()
                .name(...CAKE_STALL_NAMES)
                .where(l => stealFromOp(l) && Tile.from(l.tile()).distanceTo(CAKE_STALL_STAND) <= CAKE_STALL_MARKET_RADIUS)
                .nearest() ??
            Locs.query()
                .where(
                    l =>
                        ...
                        /stall/i.test(l.name ?? '') &&
                        stealFromOp(l) &&
                        Tile.from(l.tile()).distanceTo(CAKE_STALL_STAND) <= CAKE_STALL_MARKET_RADIUS
                )
                .nearest()
        );
    }
    ...
    async stealCakesFromStall() {
        ...
        if (!stall) {
            ...
            if (here && Tile.from(here).distanceTo(CAKE_STALL_STAND) > 3) {
                await Traversal.walkTo(CAKE_STALL_STAND, { radius: 2, timeoutMs: 12_000 });
            }
            await Execution.delayTicks(2);
            return;
        }
    ...
    async stealCakesFinishFromOpenBank() {
        ...
        if (need > 0) {
            ...
            for (const { name, take } of plan) {
                this.log(`steal cakes: withdrawing ${take}× ${name}`);
                if (!(await Bank.withdrawX(name, take))) {
        ...
        this.status = `returning to ${cfg.name}`;
        this.log(
            `steal cakes trip ${this.stealCakesTrips} done — ${this.stealCakesBanked} cakes, ` +
                `${this.foodCount()} food, walking to ${cfg.anchor.x},${cfg.anchor.z}`
        );
        await Traversal.walkResilient(cfg.anchor, {
            radius: 4,
            log: m => this.log(`  ${m}`)
    ...
    beginCatchCook(context) {
        ...
        this.startReady = true;
        this.log(
            `${context}: no Lobster in bank — catch & cook ${CATCH_COOK_GOAL} at Catherby ` +
                `(Cage+Harpoon, cook on Range, then return)`
        );
    ...
    async catchCookTick() {
        ...
        if (Inventory.isFull()) {
            if (cookableLobsterCount() > 0) {
                ...
                this.log(
                    `catch & cook: full inv (${cookableLobsterCount()} raw) — cooking on Range ` +
                        `(${this.catchCookCooked}/${CATCH_COOK_GOAL})`
                );
                await this.catchCookCookLoad();
                return;
            }
        ...
        if (!spot) {
            ...
            if (Tile.from(here).distanceTo(CATHERBY_SHORE) > CATHERBY_STAND_RADIUS) {
                await Traversal.walkTo(CATHERBY_SHORE, { radius: 2, timeoutMs: 12_000 });
            }
            await Execution.delayTicks(3);
            return;
        }
    ...
    catchCookFindSpot() {
        return Npcs.query()
            ...
            .where(n => Tile.from(n.tile()).distanceTo(CATHERBY_SHORE) <= CATHERBY_SHORE_LEASH)
            .nearest();
    }
    ...
    async catchCookCage(spot) {
        ...
        await Execution.delayUntil(
            () =>
                rawLobsterCount() > before ||
                isAnimating() ||
                ChatDialog.canContinue() ||
                !this.catchCookFindSpot(),
            8000
        );
        this.catchCookNoteCatches();
    }
    ...
    async catchCookPrepPot() {
        if (
            catchCookHasPot() &&
            cookedLobsterCount() === 0 &&
            rawLobsterCount() === 0 &&
            burntFishCount() === 0 &&
            !Inventory.items().some(i => i.name && !isCatchCookKeep(i.name))
        ) {
    ...
    async catchCookWithdrawPot() {
        ...
        const op =
            (typeof withdrawOp === 'function' ? withdrawOp(potBank.ops, '1') : null) ??
            'Withdraw-1';
    ...
    async catchCookHandleHarry() {
        ...
        if (!catchCookHasPot() && invCoins() >= POT_COST) {
            this.log(`catch & cook: Shop.buy 1× ${POT_NAME}`);
            const bought = await Shop.buy(POT_NAME, 1);
            if (bought > 0) {
                this.log(`catch & cook: bought ${bought}× ${POT_NAME} from Harry`);
            } else {
    ...
    catchCookFindRange() {
        return (
            Locs.query()
                ...
                .where(l => Tile.from(l.tile()).distanceTo(CATHERBY_RANGE_LOC) <= CATHERBY_RANGE_LEASH)
                .nearest() ??
    ...
    async catchCookWalkToRange() {
        ...
        if (here && Tile.from(here).distanceTo(CATHERBY_RANGE_STAND) > 2) {
            await Traversal.walkTo(CATHERBY_RANGE_STAND, { radius: 1, timeoutMs: 12_000 });
        }
        if (!this.catchCookFindRange()) {
            await Traversal.walkTo(CATHERBY_RANGE_LOC, { radius: 1, timeoutMs: 8_000 });
            await this.openNearbyDoor({ within: 3 });
        }
    }
    ...
    async catchCookChooseCookProduct() {
        const products =
            typeof ChatDialog.makeProducts === 'function' ? ChatDialog.makeProducts() : [];
        const raw = lastCookableRawLobster();
        const hint =
            products.find(p => (p ?? '').toLowerCase().includes('lobster')) ??
            products[0] ??
            raw?.name ??
            null;
    ...
    async catchCookCookLoad() {
        ...
        const started = await Execution.delayUntil(
            () =>
                cookableLobsterCount() < beforeCookable ||
    ...
    async catchCookBankOrFinish() {
        ...
        this.log(
            `catch & cook: banking` +
                (rawLobsterCount() ? ` ${rawLobsterCount()} raw` : '') +
                (cookedLobsterCount() ? ` ${cookedLobsterCount()} cooked` : '') +
                ` — ${this.catchCookCooked}/${CATCH_COOK_GOAL}`
        );
    ...
    async catchCookFinishFromOpenBank() {
        ...
        this.log(
            `catch & cook: done ${this.catchCookCooked} cooked (${this.catchCookCaught} caught) — ` +
                `deposit pot, withdraw food, return to Ardougne`
        );
        await Bank.depositAllMatching(name => (name ?? '').toLowerCase() !== 'coins');
        await Execution.delayTicks(1);
        ...
        this.status = `returning to ${cfg.name}`;
        this.log(
            `catch & cook trip ${this.catchCookTrips} done — ${this.foodCount()} food, ` +
                `walking to ${cfg.anchor.x},${cfg.anchor.z}`
        );
        await Traversal.walkResilient(cfg.anchor, {
            radius: 4,
            log: m => this.log(`  ${m}`)
    ...
    beginMuleHandoff() {
        ...
        this.nextMuleTradeRequestAtMs = 0;
        this.log(
            `mule: bank ${fmtGp(this.bankGp)}gp + pack ${fmtGp(invCoins())}gp ` +
                `(id ${COINS_ID}) > ${fmtGp(MULE_GP_THRESHOLD)} — ` +
                `withdraw ALL Coins at ${MULE_STAND.x},${MULE_STAND.z} and trade to ${MULE_NAME}`
        );
    ...
    async muleWithdrawBankCoins() {
        ...
        if (!here || Tile.from(here).distanceTo(MULE_STAND) > BANK_MULE_LEASH) {
            ...
            if (!this.muleAnnounced) {
                ...
                this.log(
                    `mule: bank ${fmtGp(this.bankGp)}gp → ${MULE_STAND.x},${MULE_STAND.z} ` +
                        `withdraw ALL Coins then Trade with ${MULE_NAME}`
                );
            }
    ...
    async requestMuleTrade() {
        ...
        if (!partner) {
            ...
            if (Tile.from(here).distanceTo(MULE_STAND) > 2) {
                await Traversal.walkTo(MULE_STAND, { radius: 2, timeoutMs: 8_000 });
            }
            await Execution.delayTicks(3);
            return;
        }
    ...
    async driveMuleTrade() {
        ...
        while (typeof Trade !== 'undefined' && Trade.active()) {
            ...
            if (!offered) {
                ...
                await Execution.delayUntil(
                    () =>
                        Trade.myOffer().some(i => isCoinsItem(i)) ||
    ...
    override onPaint(ctx: CanvasRenderingContext2D): void {
        ...
        const hpMode = this.foodEnabled()
            ? `eat ≤ ${this.eatAtHp}  ·  ${this.describeFoodPrefs()} ${this.foodCount()}/${this.foodWithdraw}`
            : this.waitForHp
        ...
        const lines = [
            ...
            `Target ${this.targetPref === TARGET_AUTO ? 'Auto → ' : ''}${targetNames(cfg).join(' / ')}  ·  Thieving ${Skills.level('thieving')}`,
            `HP ${hp}/${Skills.level('hitpoints')}  ·  ${hpMode}`,
            this.stealCakesActive
                ? `steal cakes ${this.stealCakesBanked}/${CAKE_STALL_GOAL}  carried ${carriedCakes()}`
                : this.stealCakesWanted()
                  ? `steal cakes on  ·  ${this.stealCakesTrips} Baker trips`
                  : this.catchCookActive
                    ? `catch & cook ${this.catchCookCooked}/${CATCH_COOK_GOAL}  caught ${this.catchCookCaught}  pot ${catchCookHasPot() ? 'yes' : 'NO'}`
                    : this.catchAndCookWanted()
                      ? `catch & cook on  ·  ${this.catchCookTrips} Catherby trips`
                      : `catch & cook off`,
            `steals ${this.steals}  fails ${this.fails}  eats ${this.eats}  banks ${this.bankTrips}`,
            `GP stolen ${fmtGp(this.gpStolen)}  ·  bank ${fmtGp(this.bankGp)}gp`,
            this.muleHandoffActive
                ? `mule handoff → ${MULE_NAME}  pack ${fmtGp(invCoins())}gp`
                : this.muleWanted()
                  ? `mule bank ${fmtGp(this.bankGp)}/${fmtGp(MULE_GP_THRESHOLD)} → ${MULE_NAME}  sent ${fmtGp(this.muled)}  trips ${this.muleTrips}`
                  : `mule off  ·  bank ${fmtGp(this.bankGp)}gp`,
...
export default defineBot({
    ...
    settingsSchema: {
        ...
        muleOn: {
            ...
            group: 'Mule',
            help:
                'On by default. Keep pickpocketing and banking Coins (item id 995). Once banked gp exceeds 100,000, withdraw ALL coins at East Ardougne bank (2653,3284) and trade them to Ben. After the trade, resume thieving. Pause → Edit parameters to toggle without stopping.'
        }
    },
