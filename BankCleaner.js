 ...
 *
 * Load URL: https://benzymee.github.io/RS2B2TScripts/BankCleaner.js
 */
...
function isCookware(n) {
  return n === "bowl" || n === "empty bowl" || n === "pie dish" || n === "empty pot" || n === "pot" || n === "empty jug" || n === "jug" || n === "jug of water" || n === "empty cup" || n === "basket" || n === "empty basket" || n === "sack" || n === "empty sack";
}
function isEdible(n) {
  if (hasWord(n, "key")) {
    return false;
  }
  return fishRank(n) < 90 || indexIn(FOOD_BAKED, n) < 90 || indexIn(FOOD_MEAT, n) < 90 || indexIn(FOOD_DRINK, n) < 90 || indexIn(FOOD_PRODUCE, n) < 90 || isCookware(n) || n.includes("burnt") && (fishRank(n) < 90 || hasWord(n, "fish") || hasWord(n, "meat"));
}
function foodSub(n) {
  if (n.includes("burnt")) {
    const fish2 = fishRank(n);
    return 80 + (fish2 < 90 ? fish2 : 0);
  }
...
function classify(name, id) {
  ...
  if (/\b(pickaxe|hatchet|tinderbox|hammer|chisel|needle|shears|spade|rake|secateurs|harpoon|lobster pot|small fishing net|big fishing net|fishing rod|fly fishing rod|barbarian rod|saw|glassblowing|pestle|crucible|mould)\b/.test(
    n
  ...
  const isBow = hasWord(n, "shortbow") || hasWord(n, "longbow") || hasWord(n, "crossbow") || hasWord(n, "bow") || n.endsWith("bow");
  ...
  const wornHide = hasWord(n, "body") || hasWord(n, "chaps") || hasWord(n, "vambraces") || hasWord(n, "vambrace") || hasWord(n, "coif") || hasWord(n, "shield");
  if (/\b(cowhide|leather|dragonhide|d hide|thread|wool|ball of wool|silk|molten glass|soda ash|bucket of sand)\b/.test(n) && !wornHide) {
...
function walkCache() {
  ...
  const cap = typeof OT.count === "number" ? OT.count : 8e3;
  let miss = 0;
...
class BankCleaner extends LoopingBotBase {
  ...
  async onStart() {
    ...
    const filled = cache.counts.map((n, i) => n > 0 ? `${CAT_LABELS[i]} ${n}` : null).filter(Boolean).slice(0, 8).join(", ");
    this.log(
      `Benzyme's Bank Cleaner \u2014 slot-swap sort (INV_BUTTOND), not mouse-drag. Cache ${cache.named} items. ${filled}`
    );
    this.status = "ready";
  }
  onStop() {
    ...
    for (let dest = 0; dest < wanted.length; dest++) {
      ...
      if (moved % RESYNC_EVERY === 0) {
        if (typeof Bank.waitSnapshotAfter === "function" && gen >= 0) {
          await Bank.waitSnapshotAfter(gen, 2e3);
        } else {
          await Execution.delayTicks(1);
        }
  ...
  onPaint(ctx) {
    ...
    const lines = [
      ...
      `time ${elapsed} \xB7 ${this.status}`,
      `stacks ${this.stacks} \xB7 swaps ${this.swaps}`,
      `cache ${this.catalogNamed} items \xB7 swap packets (no drag)`,
...
var BankCleaner_default = defineBot({
  name: SCRIPT_NAME,
  version: SCRIPT_VERSION,
  category: "Utility",
  tags: ["bank", "sort", "utility", "cleaner"],
  description: "Opens the nearest bank and sorts stacks by type, such as runes, ores, food, and gear.",
  settingsSchema: {
    depositFirst: {
      type: "boolean",
      default: true,
...
export {
  BankCleaner_default as default
};
