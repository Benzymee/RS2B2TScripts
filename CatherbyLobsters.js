...
if (!abi) {
  throw new Error("CatherbyLobsters: globalThis.__rs2b0t missing \u2014 load inside rs2b0t bot.html");
}
if (abi.apiVersion !== SUPPORTED_API_VERSION) {
  throw new Error(
    `CatherbyLobsters: ABI ${abi.apiVersion} != supported ${SUPPORTED_API_VERSION}`
  );
}
const {
  defineBot,
...
const SCRIPT_NAME = "CatherbyLobsters";
...
const POT_COST = 20;
const HARRY_BUY_RAW = /* @__PURE__ */ new Set(["raw lobster"]);
function fmtXph(n) {
  if (n >= 1e5) {
...
const PAINT_FONT_ID = "benzyme-catherby-lobsters-font-v1";
...
function inventoryOnlyPot() {
  const items = Inventory.items().filter((i) => i.name);
  if (items.length === 0) {
    return false;
  }
  return items.every((i) => (i.name ?? "").toLowerCase() === "lobster pot");
}
function readyToFish() {
  return hasPot() && inventoryOnlyPot() && nothingEquipped();
}
...
class CatherbyLobsters extends LoopingBotBase {
  status = "starting";
  startedAt = 0;
  fishXpAtStart = 0;
  cookXpAtStart = 0;
  ...
  bankTrips = 0;
  sellTrips = 0;
  /** False until start bank: deposit all + withdraw Lobster pot finishes. */
  startReady = false;
  ...
  async onStart() {
    ...
    this.log(
      `CatherbyLobsters @ ${ANCHOR.x},${ANCHOR.z} \u2014 start always banks everything (deposit \u2192 unequip \u2192 deposit \u2192 withdraw Lobster pot); Cage on Harry/mid first, cape via sand (never east rocks); cook on way: ${this.cookOnWay ? "on" : "off"}; sell to Harry: ${this.sellToHarry ? "on" : "off"}`
    );
  ...
  onStop() {
    ...
    this.log(
      `stopped \u2014 caught ${this.caught}, cooked ${this.cooked}, sold ${this.sold}, bank ${this.bankTrips}, sell trips ${this.sellTrips} (${this.status})`
    );
  }
  ...
  async loop() {
    ...
    if (needsGear()) {
      ...
      this.log("missing Lobster pot \u2014 bank first, then Harry if needed");
  ...
  findCageSpot() {
    ...
    return this.pickBestCageSpot(npcList(q.where((n) => this.spotIsFishable(n.tile()))));
  }
  async cageSpot(spot) {
    ...
    await Execution.delayUntil(
      () => rawFishCount() > before || Game.animating() || ChatDialog.canContinue() || this.cantReach || Inventory.isFull() || !this.findCageSpot(),
      8e3
    );
    this.noteCatches();
    if (this.cantReach) {
  ...
  async lootPotFromGround() {
    ...
    const ground = GroundItems.query().name(POT_NAME).within(12).nearest() ?? GroundItems.query().where((g) => isPotGroundName(g.name)).within(12).nearest();
    if (!ground) {
      return false;
    }
    const before = Inventory.used();
    await ground.interact("Take");
    return await Execution.delayUntil(() => hasPot() || Inventory.used() > before, 6e3) && hasPot();
  }
  /** Deposit every inventory slot (including pots) while the bank is open. */
   ...
   * withdraw Lobster pot only → shore. Never starts fishing until that finishes.
   * If bank has no pot, withdraws coins and buys one from Harry.
   */
  async prepStartInv() {
    if (!Bank.isOpen()) {
      this.status = "start: bank";
      this.log(
        "start: opening bank \u2014 deposit everything, then withdraw Lobster pot"
      );
    ...
    if (!await this.withdrawPotOnlyFromOpenBank()) {
      ...
      this.log("start: no Lobster pot in bank \u2014 buying from Harry");
      await this.buyPotFromHarryAndReturn();
      if (hasPot()) {
    ...
    if (!readyToFish()) {
      this.log("start: expected pot-only after withdraw \u2014 retrying");
      await Execution.delayTicks(2);
      return;
    }
    await Bank.close();
    ...
    this.lastRawSeen = rawFishCount();
    this.status = "returning to shore";
    this.log("start done \u2014 Lobster pot only; walking to shore");
    await Traversal.walkResilient(ANCHOR, {
      radius: 3,
      log: (m) => this.log(`  ${m}`)
    });
  }
  /** Withdraw one Lobster pot from an already-open bank (no coin fallback). */
  async withdrawPotOnlyFromOpenBank() {
    if (!Bank.isOpen()) {
      return false;
    }
    if (hasPot()) {
  ...
  async bankRestockAndReturn() {
    ...
    this.status = "banking";
    this.log(
      `banking` + (raw ? ` ${raw} raw` : "") + (cooked ? ` ${cooked} cooked` : "") + (burntCount() ? ` ${burntCount()} burnt` : "") + ` \u2014 restock Lobster pot`
    );
    this.lastRawSeen = 0;
    await this.peelOffEastBeach();
  ...
  async buyPotInOpenShop() {
    ...
    this.log(`Shop.buy 1\xD7 ${POT_NAME}`);
    const bought = await Shop.buy(POT_NAME, 1);
    if (bought > 0) {
      this.log(`bought ${bought}\xD7 ${POT_NAME} from Harry`);
    } else {
  ...
  onPaint(ctx) {
    ...
    const lines = [
      `Benzyme's Catherby Lobsters  Fish ${Skills.level("fishing")}  Cook ${Skills.level("cooking")}`,
      ...
      `pot ${hasPot() ? "yes" : "NO"}  bank ${this.bankTrips}` + (this.sellToHarry || this.sellTrips > 0 ? `  sells ${this.sellTrips}` : "") + `  Fish XP ${fmtXph(fishXph)}/hr` + (this.cookOnWay || cookXp > 0 ? `  Cook XP ${fmtXph(cookXph)}/hr` : "")
    ];
    ctx.font = PAINT_FONT;
    let maxW = 0;
...
var CatherbyLobsters_default = defineBot({
  name: SCRIPT_NAME,
  version: "1.0.0",
  category: "Fishing",
  tags: ["fishing", "catherby", "lobster", "cage", "bank", "cook", "harry"],
  description: "Cage-fishes lobsters at Catherby. Buys a lobster pot from Harry if needed. Optional cook on the Range next to the bank, or sell raw lobster to Harry.",
  settingsSchema: {
    cookOnWay: {
      type: "boolean",
      default: true,
      ...
      group: "Cooking",
      help: "When the pack is full, cook Raw lobster on the Catherby bank-house Range (Cooking 40+), drop burnt, then bank or sell to Harry"
    },
    sellToHarry: {
      type: "boolean",
      default: false,
      label: "Sell to Harry",
      group: "Sell",
      help: "Sell Raw lobster to Harry at the Catherby fishing shop instead of banking, then return to cage fishing. Cooked leftovers (if cook-on-way is on) still bank. Restocks Lobster pot from bank; buys one from Harry when bank has none."
    }
  },
  create: () => new CatherbyLobsters()
});
export {
  CatherbyLobsters_default as default
};
