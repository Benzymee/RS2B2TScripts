...
/**
 * Catherby Cage+Harpoon hops (fishing_movement_catherby_enum):
 *   Harry sand  2836–2838,3431
 *   mid beach    2844–2846,3429
 *   east rocks   2853–2855,3423  (unreachable — skip)
 *   lobster cape 2859–2860,3426  (dead-end — sand corridor only)
 */
...
/** Neck of the peninsula — west of cape, north of rocks. */
const EAST_APPROACH = new Tile(2856, 3429, 0);
/** Furthest safe scan stand — sees cape hops, not the last dead-end tiles. */
...
function isEastRocksHop(tile: TileLike | null | undefined) {
    if (!tile) {
        return false;
    }
...
/** Mule partner — receive noted tuna/swordfish at Catherby bank. */
const MULE_NAME = 'eoc';
const MULE_TRADE_RANGE = 2;
/** Stay inside Catherby bank while waiting / trading. */
const BANK_MULE_LEASH = 8;
/** Banked tuna ≥ this OR swordfish ≥ this (each type counted separately, not combined). */
const MULE_THRESHOLD = 1000;
...
/** Bank names to withdraw-all as notes during a mule handoff. */
...
/** Harry's Harpoon baseline cost (gp). */
const HARPOON_COST = 5;

/** Raw names Harry buys that we catch (harpoon on Cage+Harpoon). */
const HARRY_BUY_RAW = new Set(['raw tuna', 'raw swordfish']);
...
function isKeepGear(name: string | undefined) {
    if (!name) {
        return false;
    }
...
function isRawHarpoonFish(name: string | undefined) {
    if (!name) {
        return false;
    }
...
function isCookedHarpoonFish(name: string | undefined) {
    if (!name) {
        return false;
    }
...
function isHarrySellable(name: string | undefined) {
    if (!name) {
        return false;
    }
 ...
 * Tuna: Fishing 35 / Cooking 30. Swordfish: Fishing 50 / Cooking 45.
 */
const COOK_LEVEL = {
    tuna: 30,
    swordfish: 45
...
function isCoins(name: string | undefined) {
...
function cageOp(actions: any) {
    return actions.find(a => /^cage$/i.test(a)) ?? null;
}

function harpoonOp(actions: any) {
    return actions.find(a => /^harpoon$/i.test(a)) ?? null;
}

/** Tuna/swordfish hops — Cage + Harpoon (never Net+Bait / Lure+Bait). */
function isCageHarpoonSpot(actions: any) {
...
function isHarpoonGroundName(name: string | undefined) {
    if (!name) {
        return false;
    }
    return name.toLowerCase() === 'harpoon';
...
function countByExactName(name: string | undefined) {
    const want = (name ?? '').toLowerCase();
    return countMatching(n => (n ?? '').toLowerCase() === want);
}

/** Cooked or raw tuna/swordfish — what we mule (never burnt). */
function isMuleFish(name: string | undefined) {
    return isRawHarpoonFish(name) || isCookedHarpoonFish(name);
}

/** Try ObjType.certtemplate when the client exposes it (true = bank note). */
function certIsNote(id: number) {
    try {
        const OT =
            globalThis.ObjType ??
...
/**
 * Noted tuna/swordfish: certtemplate when available; else stack count > 1
 * (unnoted fish never stacks).
 */
function isNotedMuleFishItem(item: InvItem | null | undefined) {
...
function isUnnotedMuleFishItem(item: InvItem | null | undefined) {
    return !!item && isMuleFish(item.name) && !isNotedMuleFishItem(item);
}

/** depositAllMatching: unnoted tuna/swordfish only (never notes, never the harpoon). */
function isUnnotedMuleFishDeposit(name: string | undefined, id: number) {
    if (!isMuleFish(name)) {
        return false;
    }
...
function muleNamesForKind(kind: string | null | undefined) {
    if (kind === 'tuna') {
...
function muleNamesForKinds(kinds: any) {
    return kinds.flatMap(muleNamesForKind);
}

function notedCountForKind(kind: string | null | undefined) {
    return Inventory.items()
        .filter(i => isNotedMuleFishItem(i) && fishKind(i.name) === kind)
        .reduce((n, i) => n + Math.max(1, i.count), 0);
}

function unnotedCountForKind(kind: string | null | undefined) {
    return Inventory.items()
        .filter(i => isUnnotedMuleFishItem(i) && fishKind(i.name) === kind)
        .reduce((n, i) => n + Math.max(1, i.count), 0);
}

function fmtMuleCounts(tuna: any, sword: any) {
...
class CatherbySwordfish extends LoopingBotBase {
    ...
    override async onStart(): Promise<void> {
        ...
        this.log(
            `CatherbySwordfish @ ${ANCHOR.x},${ANCHOR.z} — Harpoon on Cage+Harpoon only; ` +
                `Harry/mid first, cape via sand (never east rocks); ` +
                `withdraw Harpoon first; tuna/swordfish cook-aware; ` +
                `cook on way: ${this.cookOnWay ? 'on' : 'off'}; ` +
                `sell to Harry: ${this.sellToHarry ? 'on' : 'off'}; ` +
                `mule: ${this.muleWanted() ? `on (≥${MULE_THRESHOLD} tuna OR ≥${MULE_THRESHOLD} swordfish → ${MULE_NAME})` : 'off'}`
        );
        if (hasHarpoon()) {
    ...
    override onStop(): void {
        ...
        this.log(
            `stopped — caught ${this.caught}, cooked ${this.cooked}, sold ${this.sold}, ` +
                `muled ${this.muled}, bank ${this.bankTrips}, sell trips ${this.sellTrips}, ` +
                `mule trips ${this.muleTrips} (${this.status})`
        );
    }
    ...
    override async loop(): Promise<void> {
        ...
        if (!this.startReady || needsGear()) {
            ...
            this.log(
                !this.startReady
                    ? 'start: withdrawing Harpoon from bank (Harry if missing)'
                    : 'missing Harpoon — bank first, then Harry if needed'
            );
        ...
        if (Inventory.isFull()) {
            ...
            // Full of junk / unrelated items — bank everything except Harpoon (+ coins).
        ...
        if (
            this.muleWanted() &&
            (this.lastBankTuna >= MULE_THRESHOLD || this.lastBankSwordfish >= MULE_THRESHOLD)
        ) {
            this.log(
    ...
    async disposeCatchAndReturn(): Promise<any> {
    ...
    async walkBeachEast(): Promise<any> {
    ...
    async walkBeachToward(dest: TileLike | null | undefined): Promise<any> {
        const here = Game.tile();
        if (!here || !dest) {
            return;
        }
    ...
    async harpoonSpot(spot: Npc): Promise<any> {
    ...
    async lootHarpoonFromGround(): Promise<any> {
        if (hasHarpoon()) {
            return true;
        }
        const ground =
            GroundItems.query().name(HARPOON_NAME).within(12).nearest() ??
            GroundItems.query()
                .where(g => isHarpoonGroundName(g.name))
                .within(12)
                .nearest();
        if (!ground) {
            return false;
    ...
    async chooseCookProduct(): Promise<any> {
        ...
        const frag =
            kind === 'swordfish' ? 'swordfish' : kind === 'tuna' ? 'tuna' : null;
        const batch = frag
    ...
    async sellToHarryAndReturn(): Promise<any> {
    ...
    async handleOpenHarry(): Promise<any> {
     ...
     */
    async bankRestockAndReturn(): Promise<any> {
        ...
        await Banking.bankNearest({
            ...
            deposit: name => {
                // Bank fish and any unrelated junk — keep only Harpoon (+ coins for Harry).
     ...
     */
    async buyHarpoonFromHarryAndReturn(): Promise<any> {
        if (hasHarpoon()) {
            this.startReady = true;
            this.status = 'returning to shore';
            await Traversal.walkResilient(ANCHOR, {
    ...
    async buyHarpoonInOpenShop(): Promise<any> {
    ...
    async withdrawCoinsForHarpoon(): Promise<any> {
    ...
    async withdrawGearFromOpenBank(): Promise<any> {
        if (!Bank.isOpen()) {
            return hasHarpoon();
        }
        await Execution.delayUntil(() => Bank.loaded() || !Bank.isOpen(), 3000);
    ...
    bankCountForKind(kind: string | null | undefined) {
        return kind === 'swordfish' ? this.bankSwordfishCount() : this.bankTunaCount();
    }

    bankCountForKinds(kinds: any) {
        return kinds.reduce((n, k) => n + this.bankCountForKind(k), 0);
    }

    beginMuleHandoff({ alreadyAtBank = false }: { alreadyAtBank?: boolean } = {}) {
        if (this.muleHandoffActive) {
            return;
        }
        ...
        this.nextMuleTradeRequestAtMs = 0;
        this.log(
            `mule: handoff tuna/swordfish that are ≥${MULE_THRESHOLD} each to ${MULE_NAME}` +
    ...
    async resumeFishingAfterMule(reason: string): Promise<any> {
        this.muleHandoffActive = false;
        this.muleReadyToTrade = false;
        this.muleAnnounced = false;
        this.startReady = true;
    ...
    bankCountByName(name: string | undefined) {
        const want = (name ?? '').toLowerCase();
        const items = Bank.items?.() ?? [];
        const fromItems = items
    ...
    notedWithdrawAllOp(bankItem: any) {
    ...
    async ensureBankNoteMode(on: boolean): Promise<any> {
    ...
    async depositUnnotedMuleFish(reason: string): Promise<any> {
    ...
    async withdrawAllMuleFishNoted(kinds = this.kindsReadyToMule()) {
        ...
        this.log(
            `mule: withdrawing ALL ${kinds.join(' + ')} as notes (${banked}) — ` +
                `leaving the other type in bank if it is under ${MULE_THRESHOLD}`
        );
     ...
     */
    async muleBankNotedFish(): Promise<any> {
        this.status = 'mule: banking';
        if (!this.muleAnnounced) {
            this.muleAnnounced = true;
            this.log(
                `mule: withdraw noted tuna OR swordfish (≥${MULE_THRESHOLD} each, not combined) → ` +
                    `trade to ${MULE_NAME} at Catherby bank → resume fish`
            );
        }
        ...
        this.log(
            `mule: live ${fmtMuleCounts(this.bankTunaCount(), this.bankSwordfishCount())} ` +
                `bank ${banked} noted-pack ${notedHeld} unnoted-pack ${unnotedHeld}`
        );
        ...
        if (kinds.length === 0) {
            ...
            await this.resumeFishingAfterMule(
                `need ≥${MULE_THRESHOLD} tuna or ≥${MULE_THRESHOLD} swordfish before mule ` +
                    `(have ${fmtMuleCounts(this.tunaAvailableCount(), this.swordfishAvailableCount())})`
            );
            return;
        }
        ...
        const readyNoted =
            (kinds.includes('tuna') && notedNowTuna >= MULE_THRESHOLD) ||
            (kinds.includes('swordfish') && notedNowSword >= MULE_THRESHOLD);

        // Already holding enough notes of a ready type; leave under-threshold type in bank.
        ...
        if (notedAfterTuna < MULE_THRESHOLD && notedAfterSword < MULE_THRESHOLD) {
            if (leftoverReadyAfter > 0) {
                this.log(
                    `mule: pack ${fmtMuleCounts(notedAfterTuna, notedAfterSword)} with ` +
                        `${leftoverReadyAfter} ready-type still in bank — retrying`
                );
                await this.ensureBankNoteMode(true);
                await Execution.delayTicks(2);
                return;
            ...
            await this.resumeFishingAfterMule(
                `need ≥${MULE_THRESHOLD} tuna or ≥${MULE_THRESHOLD} swordfish before mule ` +
                    `(have ${fmtMuleCounts(notedAfterTuna, notedAfterSword)})`
            );
            return;
        }
    ...
    async requestMuleTrade(): Promise<any> {
        const here = Game.tile();
        if (!here || Tile.from(here).distanceTo(BANK_STAND) > BANK_MULE_LEASH) {
     ...
     */
    async muleWaitAndAcceptScreen(screen: any): Promise<any> {
    ...
    async driveMuleTrade(): Promise<any> {
    ...
    override onPaint(ctx: CanvasRenderingContext2D): void {
        ...
        const muledPh = hrs > 0.008 ? this.muled / hrs : 0;
        ...
        const lines = [
            ...
            `caught ${this.caught} (${fmtXph(caughtPh)}/hr)  cooked ${this.cooked} (${fmtXph(cookedPh)}/hr)` +
                (this.sellToHarry || this.sold > 0
                    ...
                    : ''),
            `harpoon ${hasHarpoon() ? 'yes' : 'NO'}  bank ${this.bankTrips}` +
        ...
        if (this.muleWanted() || this.muleHandoffActive || this.muled > 0) {
            lines.push(
                this.muleHandoffActive
                    ? `mule handoff → ${MULE_NAME}  noted ${fmtMuleCounts(notedCountForKind('tuna'), notedCountForKind('swordfish'))}`
                    : `mule ${fmtMuleCounts(this.lastBankTuna, this.lastBankSwordfish)} → ${MULE_NAME}  sent ${this.muled} (${fmtXph(muledPh)}/hr)  trips ${this.muleTrips}`
            );
        }
...
export default defineBot({
    ...
    settingsSchema: {
        ...
        cookOnWay: {
            ...
            group: 'Cooking',
            help:
                'When the pack is full, cook Raw tuna (Cooking 30+) and Raw swordfish (Cooking 45+) on the Catherby bank-house Range, drop burnt, then bank or sell to Harry'
        },
        sellToHarry: {
            type: 'boolean',
            default: false,
            ...
            group: 'Sell',
            help:
                'Sell Raw tuna / Raw swordfish to Harry at the Catherby fishing shop instead of banking, then return to harpoon fishing. Cooked leftovers (if cook-on-way is on) still bank. Restocks Harpoon from bank; buys one from Harry when bank has none.'
        }
    },
