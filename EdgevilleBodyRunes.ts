...
function T(x: number, z: number, level: number = 0) {
    return new Tile(x, z, level);
}

/** Edgeville bank stand (first pin of the recorded body-altar walk). */
const BANK_STAND = T(3092, 3492);
/** Mysterious ruins loc. Not a stand tile: 3053,3445 is unreachable. */
const RUINS_STAND = T(3053, 3445);
/** Last recorded pin, south of the ruins. Stand here, then use the talisman. */
const RUINS_APPROACH = T(3057, 3439);

/**
 * Recorded walkable tiles, Edgeville bank → body ruins. Reverse is the bank trip.
 * The walker paths to each pin (radius 5) and starts the next as soon as that pin is close.
 */
const BODY_ROUTE = [
    T(3092, 3492),
    T(3080, 3475),
...
function nameIs(name: string | undefined, list: any) {
    const n = normName(name);
    return list.some(s => n === normName(s));
}

function cheb(a: any, b: any) {
    if (!a || !b) {
        return 99;
    }
...
function isTalisman(name: string | undefined) {
    return nameIs(name, [TALISMAN_NAME]);
}

function isTiara(name: string | undefined) {
    return nameIs(name, [TIARA_NAME]);
}

function isEssence(name: string | undefined) {
    return nameIs(name, [...ESSENCE_NAMES, 'essence']);
}

function isBodyRune(name: string | undefined) {
    return nameIs(name, BODY_RUNE_NAMES);
}

function isNoteId(id: number) {
...
function locTile(loc: Loc | null | undefined) {
    try {
...
function portalOp(loc: Loc | null | undefined) {
...
function craftOp(loc: Loc | null | undefined) {
    const acts = locActions(loc);
    return (
...
function ruinsEnterOp(loc: Loc | null | undefined) {
    const acts = locActions(loc);
    return (
        acts.find(a => /^enter/i.test(a ?? '')) ??
        acts.find(a => /enter|use/i.test(a ?? '')) ??
        null
    );
}
...
function bankItemCount(pred: (name: string | undefined, id?: number) => boolean) {
    return bankItems()
        .filter(i => pred(i.name))
        .reduce((n, i) => n + Math.max(1, i.count ?? 1), 0);
}

function bankCountByName(name: string | undefined) {
...
async function waitBankLoaded(): Promise<any> {
    if (typeof Bank.loaded === 'function') {
        await Execution.delayUntil(() => Bank.loaded() || bankItems().length > 0, 4000);
    }
    await Execution.delayUntil(() => bankContentsReady() || !Bank.isOpen(), 4000);
    await Execution.delayTicks(1);
}

async function withdrawNamed(name: string | undefined, qty: number): Promise<any> {
    if (qty <= 0 || !Bank.isOpen() || !name) {
        return false;
    }
    const bankItem =
        (typeof Bank.items === 'function' ? Bank.items().find(i => normName(i.name) === normName(name)) : null) ??
        null;
...
function onBodyCorridor(here: TileLike | null | undefined) {
    if (!here) {
        return false;
    }
...
function viasFor(here: TileLike | null | undefined, dest: TileLike | null | undefined) {
    if (!here || !dest) {
        return [];
    }
...
function pinListTo(dest: TileLike | null | undefined, vias: any) {
...
function walkTarget(dest: TileLike | null | undefined) {
    if (dest && cheb(dest, RUINS_STAND) <= 2) {
...
function snapPinIndex(route: TileLike[], here: TileLike | null | undefined) {
...
function advancePinIndex(route: TileLike[], here: TileLike | null | undefined, pinIndex: any) {
...
function keepEntryName(name: string | undefined) {
    if (isTiara(name)) {
        return true;
    }
    if (isTalisman(name)) {
...
class EdgevilleBodyRunes extends LoopingBotBase {
    ...
    override async onStart(): Promise<void> {
        ...
        this.log(
            `EdgevilleBodyRunes v${SCRIPT_VERSION} RC ${rcLevel()} (need ${BODY_LEVEL}); ` +
                `Edgeville bank ${BANK_STAND.x},${BANK_STAND.z} → body ruins ${RUINS_STAND.x},${RUINS_STAND.z}`
        );
    ...
    noteCrafted(beforeRunes: any, beforeEssence: any) {
    ...
    async openNearbyDoor(radius: number = 3): Promise<any> {
    ...
    async walkTo(dest: TileLike | null | undefined, radius: number): Promise<any> {
    ...
    async followPin(tile: TileLike | null | undefined, radius: number = PIN_FOLLOW): Promise<any> {
        if (!tile) {
            return false;
        }
        const here0 = tileOf();
        ...
        if (typeof Traversal?.walkResilient === 'function') {
            ...
            await Traversal.walkTo(tile, { radius, timeoutMs: 20_000 });
        }
     ...
     */
    async walkPinsAhead(route: TileLike[], dest: TileLike | null | undefined, radius: number): Promise<any> {
        dest = walkTarget(dest);
    ...
    async openEdgevilleBank(): Promise<any> {
        if (Bank.isOpen()) {
        ...
        if (typeof Bank.openBooth === 'function') {
            opened = !!(await Bank.openBooth(BANK_STAND, 'Bank booth', 'Use-quickly', m =>
                this.log(`  ${m}`)
            ));
        }
        if (!opened) {
    ...
    async bankCycle(opts: any = {}): Promise<any> {
        ...
        if (!hasEntryItem()) {
            ...
            if (tiaras > 0) {
                this.log(`withdrawing 1× ${TIARA_NAME}`);
                ...
                await Execution.delayUntil(() => tiaraCount() > before || !Bank.isOpen(), 4000);
                await Execution.delayTicks(1);
            } else {
                this.log(`withdrawing 1× ${TALISMAN_NAME}`);
                ...
                await Execution.delayUntil(() => talismanCount() > before || !Bank.isOpen(), 4000);
                await Execution.delayTicks(1);
            }
            if (!hasEntryItem()) {
        ...
        this.log(`withdrawing ${take}× ${label} (bank has ${inBank})`);
        ...
        await Execution.delayUntil(() => essenceCount() > beforeEss || !Bank.isOpen(), 4000);
        await Execution.delayTicks(1);
        ...
        if (stillFree > 0 && stillBank > 0) {
            ...
            await Execution.delayUntil(() => invFree() <= 0 || !Bank.isOpen(), 3000);
            await Execution.delayTicks(1);
        }

        if (!hasEssence()) {
            this.log(
                `withdraw clicked but inventory still has 0 essence ` +
                    `(bank ${bankEssenceTotal()}, last saw ${this.lastBankEssence}), retrying`
            );
            await Execution.delayTicks(3);
            return;
        }
     ...
     */
    async confirmBankEssenceOrRetry(): Promise<any> {
        if (!Bank.isOpen()) {
            return -1;
        }
        ...
        if (!bankContentsReady()) {
            ...
            if (!bankContentsReady()) {
                if (this.lastBankEssence <= 0) {
                    ...
                    this.log(
                        `empty bank after wait (confirm ${this.emptyEssenceConfirms}/3), ` +
                            (this.emptyEssenceConfirms < 3 ? 'retrying' : 'giving up')
                    );
        ...
        this.log(
            `bank shows 0 essence (confirm ${this.emptyEssenceConfirms}/3, ` +
                `items ${items}, last saw ${this.lastBankEssence}), ` +
                (this.emptyEssenceConfirms < 3 ? 'retrying' : 'giving up')
        );
        if (this.emptyEssenceConfirms < 3) {
            await Bank.close();
    ...
    async craftAtAltar(): Promise<any> {
        if (!hasEssence()) {
            return;
        }
    ...
    override onPaint(ctx: CanvasRenderingContext2D): void {
        ...
        const here = tileOf();
        const where = inAltarInterior(here)
            ? 'altar'
            : nearRuins(here)
              ? 'ruins'
              : nearBank(here)
        ...
        const lines = [
            ...
            `Body runes · RC ${rcLevel()} · ${where}`,
            `body ${this.crafted} · ${fmtXph(rph)}/hr · trips ${this.bankTrips}`,
            `ess ${essenceCount()} · ${entry}`,
            `runecraft: ${fmtXph(xph)} xp/hr  (+${Math.round(xp)} xp)`,
            `pack ${28 - invFree()}/28 · Edgeville ↔ ruins`
        ];
