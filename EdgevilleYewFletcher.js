...
if (!abi) {
    throw new Error('EdgevilleYewFletcher: globalThis.__rs2b0t missing, load inside rs2b0t bot.html');
}
if (abi.apiVersion !== SUPPORTED_API_VERSION) {
    throw new Error(
        `EdgevilleYewFletcher: ABI ${abi.apiVersion} != supported ${SUPPORTED_API_VERSION}`
    );
}
...
const SCRIPT_NAME = 'EdgevilleYewFletcher';
const SCRIPT_TITLE = "Benzyme's Edge Yews";
...
/** Edgeville yew grove south of the bank. */
const ANCHOR = new Tile(3087, 3476, 0);
const LEASH = 16;
const TREE_NAME = 'Yew';
...
/** Edgeville bank stand (same pin as WalkingBot / Edgeville booths). */
...
class EdgevilleYewFletcher extends LoopingBotBase {
    status = 'starting';
    startedAt = 0;
    wcXpAtStart = 0;
    fletchXpAtStart = 0;
    ...
    async onStart() {
        ...
        this.log(
            `${SCRIPT_TITLE} @ ${ANCHOR.x},${ANCHOR.z} (leash ${LEASH}), bank Edgeville ${EDGEVILLE_BANK.x},${EDGEVILLE_BANK.z}`
        );
        this.log(
            this.fletchEnabled()
    ...
    async loop() {
        ...
        if (Tile.from(here).distanceTo(ANCHOR) > LEASH) {
            ...
            this.log('walking back to Edgeville yews');
    ...
    /**
     * Walk to the Edgeville booths and open them. Never uses another town's bank.
    ...
    async fletchLogs(plan) {
        ...
        if (!knife) {
            this.gearReady = false;
            this.log('no Knife in inventory, will check Edgeville bank');
            await Execution.delayTicks(2);
            return;
        }
        if (!log) {
    ...
    async bankProductsAndReturn() {
        ...
        const logs = logCount();
        this.status = 'banking Edgeville';
        this.log(
            `banking at Edgeville` +
        ...
        await Banking.bankNearest({
            destination: { name: 'Edgeville', tile: EDGEVILLE_BANK },
    ...
    async restockEssentialsFromOpenBank() {
        ...
        if (!gearHasBrokenAxe() && !gearBestHeldAxe()) {
            ...
            if (best) {
                ...
                this.log(`gear: no usable axe in Edgeville bank for WC ${wc}`);
            }
        }
    ...
    async prepWcGear() {
        ...
        if (this.gearReady && this.fletchEnabled() && !gearHasKnife()) {
            this.log('gear: Knife missing, checking Edgeville bank');
            this.gearReady = false;
        }
    ...
    async bootstrapWcGear() {
        this.status = 'gear: Edgeville bank';

        if (!Bank.isOpen()) {
            this.log('gear: opening Edgeville bank for best axe / knife');
            if (!(await this.openEdgevilleBank())) {
                this.log('gear: could not open Edgeville bank, retrying');
        ...
        if (!best) {
            this.log(`gear: no usable axe in Edgeville bank/pack for WC ${wc}, waiting`);
            await Bank.close();
        ...
        if (!gearBestHeldAxe()) {
            this.log('gear: still missing axe after Edgeville bank');
            await Execution.delayTicks(5);
            return true;
        }
    ...
    async repairBrokenAxeAtBob() {
        ...
        if (after < before || !gearHasBrokenAxe()) {
            this.log('gear: axe repaired at Bob, walking back to Edgeville yews');
            const held = gearBestHeldAxe();
            if (held && !Equipment.contains(held) && canWieldTool(held, Skills.level('attack'))) {
                await Equipment.equip(held);
            }
    ...
    async prepRepairBank() {
        ...
        if (!Bank.isOpen()) {
            this.log('gear: Edgeville bank for Broken axe + 1k (rune repair)');
            if (!(await this.openEdgevilleBank())) {
                this.log('gear: could not open Edgeville bank for repair coins');
                await Execution.delayTicks(3);
                return true;
            }
        }
        ...
        if (need > 0) {
            ...
            if (have <= 0 && gearInvCoins() <= 0) {
                this.log('gear: need coins in Edgeville bank to repair at Bob, waiting');
                await Bank.close();
                await Execution.delayTicks(8);
                return true;
            }
    ...
    async walkHomeFromBob() {
        ...
        if (here && Tile.from(here).distanceTo(ANCHOR) <= LEASH) {
            this.log('gear: back at Edgeville yews');
            this.repairTrip = null;
            this.repairBanked = false;
            return false;
        }
    ...
    onPaint(ctx) {
        ...
        const lines = [
            ...
            `${plan.label}${plan.bank ? ' + Edgeville bank' : ''}`,
...
export default defineBot({
    ...
    category: 'Fletching',
    tags: ['woodcutting', 'fletching', 'yew', 'edgeville', 'shortbow', 'longbow'],
    description:
        'Chops yew trees south of Edgeville. Optional fletching into yew shortbows (u) at 65 or longbows (u) at 70, then banks at Edgeville.',
    settingsSchema: {
        fletchLogs: {
            type: 'boolean',
            default: true,
            ...
            group: 'Fletching',
            help: 'When on: fletch yew shortbows (u) at 65 / longbows (u) at 70 (needs a Knife), then bank those at Edgeville. When off: bank the logs. Missing Knife in Edgeville bank stops the script.'
        }
    },
    create: () => new EdgevilleYewFletcher()
});
