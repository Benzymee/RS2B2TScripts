...
function isFletchedLongbow(name) {
  ...
  return n.includes("long") && n.includes("bow") && (n.includes("(u)") || n.includes("unstrung"));
}
...
function shaftCount() {
  return Inventory.items().filter((i) => isShaft(i.name)).reduce((n, i) => n + Math.max(1, i.count), 0);
}
...
class FaladorTreeFletcher extends LoopingBotBase {
  status = "starting";
  startedAt = 0;
  wcXpAtStart = 0;
  fletchXpAtStart = 0;
  ...
  async onStart() {
    ...
    this.log(
      `FaladorTreeFletcher @ ${ANCHOR.x},${ANCHOR.z} (leash ${LEASH}) \u2014 ` + (this.fletchEnabled() ? `fletching ${Skills.level("fletching")} \u2192 ${plan.label}` : `banking logs (fletch off)`)
    );
    this.status = "ready";
  }
  async loop() {
  ...
  async bankProductsAndReturn() {
    ...
    this.status = "banking";
    this.log(
      `banking` + (shorts ? ` ${shorts} Shortbow` : "") + (bows - shorts > 0 ? ` ${bows - shorts} Longbow` : "") + (shafts && plan.id !== "shafts" ? ` ${shafts} arrow shafts` : "") + (!plan.fletch && logCount() ? ` ${logCount()} logs` : "") + ` (fletching ${flvl})`
    );
...
var FaladorTreeFletcher_default = defineBot({
  name: SCRIPT_NAME,
  version: "1.3.0",
  category: "Fletching",
  tags: ["woodcutting", "fletching", "falador", "trees", "arrow shafts", "shortbow", "longbow"],
  description: "Chops regular trees west of Falador and banks the logs. Optional fletching into arrow shafts, shortbows at 5, or longbows at 10. Repairs a broken axe at Bob in Lumbridge.",
  settingsSchema: {
    fletchLogs: {
      type: "boolean",
      default: true,
...
export {
  FaladorTreeFletcher_default as default
};
