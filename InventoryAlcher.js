...
function setNativeInputValue(el, value) {
  const setter = typeof window !== "undefined" && window.HTMLInputElement ? Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, "value")?.set : null;
  if (setter) {
...
async function castOnInv(item) {
  ...
  return !!actions.menuAction(TGT_BUTTON, 0, 0, com) && !!actions.menuAction(TGT_HELD, snap.id, snap.slot, snap.comId);
}
function isProtected(name, id) {
  return isNatureRune(name, id) || isCoins(name, id) || isFireRune(name, id) || isFireStaffName(name);
}
...
function installCurrentTileButton(bot) {
  ...
  const host = findParamRow(/stand coords|stand tile/i) ?? findParamRow(/stand spot/i) ?? findParamRow(/current tile/i);
  if (!host) {
    return;
  }
...
class InventoryAlcher extends LoopingBotBase {
  status = "starting";
  startedAt = 0;
  magicXpAtStart = 0;
  casts = 0;
  ...
  async loop() {
    ...
    if (!isAlchEnabled(this)) {
      this.status = "tick ALCH INVENTORY or list items";
  ...
  async alchOnce(item) {
    ...
    this.log(
      `${SPELL.label} -> ${this.currentName}` + (typeof item.id === "number" ? ` (id ${item.id})` : "") + ` x${before}`
    );
  ...
  onPaint(ctx) {
    ...
    const cph = hrs > 5e-4 ? this.casts / hrs : 0;
    const gph = hrs > 5e-4 ? this.gpAlched / hrs : 0;
    const left = alchableCount(this);
    const stand = this.standSpot && isStandSet(this.standTile) ? `stand ${this.standTile.x},${this.standTile.z}` : this.standSpot ? "stand unset" : "stand off";
    const lines = [
      `Benzyme's InventoryAlcher v${SCRIPT_VERSION}`,
      `time ${fmtElapsed(elapsed)} \xB7 ${this.status}`,
      `High Level Alchemy \xB7 Magic ${Skills.level("magic")}`,
...
var InventoryAlcher_default = defineBot({
  name: SCRIPT_NAME,
  version: SCRIPT_VERSION,
  category: "Magic",
  tags: ["magic", "alch", "high alchemy", "inventory", "utility"],
  description: "High Level Alchemy. Alch a named list of items, or tick ALCH INVENTORY to alch the pack except Nature runes, Fire runes, coins, and a fire staff. Optional stand spot. Needs Magic 55.",
  settingsSchema: {
    standSpot: {
      type: "boolean",
      default: false,
    ...
    alchInventory: {
      ...
      group: "Alch",
      help: "WARNING: ticking this box will alch everything in your inventory except Nature runes and coins. Fire runes and a fire staff are also left alone so the spell can keep casting. Leave this off unless you mean it."
    },
    alchItems: {
      type: "string",
      default: "",
...
export {
  InventoryAlcher_default as default
};
