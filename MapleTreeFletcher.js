...
function dialogOpen() {
  ...
  return typeof ChatDialog.isOpen === "function" && ChatDialog.isOpen() && typeof ChatDialog.options === "function" && ChatDialog.options().length > 0;
}
...
function logCount() {
  return Inventory.items().filter((i) => isMapleLog(i.name)).reduce((n, i) => n + Math.max(1, i.count), 0);
}
function knifeItem() {
...
class MapleTreeFletcher extends LoopingBotBase {
  ...
  async onStart() {
    ...
    this.log(
      `MapleTreeFletcher @ ${ANCHOR.x},${ANCHOR.z} (leash ${LEASH}) \u2014 ` + (this.fletchEnabled() ? `fletching ${Skills.level("fletching")} \u2192 ${plan.label}` : "banking logs (fletch off)")
    );
    this.status = "ready";
  }
  async loop() {
  ...
  async prepRepairBank() {
    ...
    const onRoute = region === "ardougne" || region === "brimhaven" || region === "musa" || region === "karamja" || region === "sarim" || region === "lumbridge" || nearBob(here);
    const canSkipBank = gearHasBrokenAxe() && (this.repairBanked || onRoute) && (gearInvCoins() >= BOAT_LEG_GP || nearBob(here) || region === "lumbridge" || region === "sarim");
    if (canSkipBank) {
  ...
  async stepSailorDialog(prefer) {
    ...
    if (typeof ChatDialog.isOpen === "function" && ChatDialog.isOpen() && typeof ChatDialog.options === "function" && ChatDialog.options().length > 0 && typeof ChatDialog.chooseOption === "function") {
  ...
  async crossGangplank() {
    const plank = Locs.query().within(10).where((l) => /gangplank/i.test(l.name ?? "")).nearest();
    if (!plank) {
      return false;
    }
    ...
    await Execution.delayUntil(() => this.movedFar(before, 3), 6e3);
    return true;
  }
  ...
  findSailor(names, stand) {
    ...
    return Npcs.query().within(18).where((n) => {
      const nm = (n.name ?? "").toLowerCase();
      return nm.includes("barnaby") || nm.includes("customs") || nm.includes("captain") || nm.includes("seaman") || nm.includes("sailor");
    }).nearest() ?? null;
  }
  ...
  stopNoKnife(context) {
    ...
    this.log(
      `${context}: no Knife in inventory or bank \u2014 stopping (Seers Village will not walk to Lumbridge; withdraw a Knife, then restart)`
    );
    stopScript();
  }
  ...
  async bankProductsAndReturn() {
    ...
    this.status = "banking";
    this.log(
      `banking` + (shorts ? ` ${shorts} Maple shortbow` : "") + (bows - shorts > 0 ? ` ${bows - shorts} Maple longbow` : "") + (logs ? ` ${logs} Maple logs` : "") + ` (fletching ${flvl})`
    );
  ...
  onPaint(ctx) {
    ...
    const lines = [
      SCRIPT_TITLE,
      `time ${fmtElapsed(snap.runtimeMs)} \xB7 ${this.status}`,
      `Woodcutting ${Skills.level("woodcutting")} \xB7 Fletching ${Skills.level("fletching")}`,
      `${plan.label}${plan.bank ? " + bank" : ""}`,
      `fletched ${snap.fletched} \xB7 logs ${logCount()} \xB7 trips ${snap.banks}`,
    ...
    lines.forEach((line, i) => {
      ...
      if (i === 0 && mapleTreeIcon && mapleTreeIcon.complete && mapleTreeIcon.naturalWidth > 0) {
...
var MapleTreeFletcher_default = defineBot({
  name: SCRIPT_NAME,
  version: SCRIPT_VERSION,
  category: "Fletching",
  tags: ["woodcutting", "fletching", "maple", "shortbow", "longbow"],
  description: "Chops maple trees near Seers Village and banks the logs. Optional fletching into maple shortbows at 50 or longbows at 55. Repairs a broken axe at Bob in Lumbridge.",
  settingsSchema: {
    fletchLogs: {
      type: "boolean",
      default: true,
...
export {
  MapleTreeFletcher_default as default
};
