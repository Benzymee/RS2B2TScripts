 ...
 *
 * Load URL: https://benzymee.github.io/RS2B2TScripts/WillowTreeFletcher.js
 */
...
function logCount() {
  return Inventory.items().filter((i) => isWillowLog(i.name)).reduce((n, i) => n + Math.max(1, i.count), 0);
}
function knifeItem() {
...
class WillowTreeFletcher extends LoopingBotBase {
  status = "starting";
  startedAt = 0;
  wcXpAtStart = 0;
  fletchXpAtStart = 0;
  ...
  async onStart() {
    ...
    this.log(
      `WillowTreeFletcher @ ${ANCHOR.x},${ANCHOR.z} (leash ${LEASH}) \u2014 ` + (this.fletchEnabled() ? `fletching ${Skills.level("fletching")} \u2192 ${plan.label}` : "banking logs (fletch off)")
    );
    this.status = "ready";
  }
  async loop() {
  ...
  async bankProductsAndReturn() {
    ...
    this.status = "banking";
    this.log(
      `banking` + (shorts ? ` ${shorts} Willow shortbow` : "") + (bows - shorts > 0 ? ` ${bows - shorts} Willow longbow` : "") + (logs ? ` ${logs} Willow logs` : "") + ` (fletching ${flvl})`
    );
  ...
  onStop() {
    this.log(
      `stopped \u2014 chopped ~${this.chopped}, fletched ~${this.fletched}, bank trips ${this.bankTrips} (${this.status})`
    );
  }
  onPaint(ctx) {
    const plan = this.currentPlan();
    ...
    const lines = [
      ...
      `time ${fmtElapsed(elapsed)}  \xB7  ${plan.label}${plan.bank ? " + bank" : ""}  \xB7  ${this.status}`,
...
var WillowTreeFletcher_default = defineBot({
  name: SCRIPT_NAME,
  version: "1.2.0",
  category: "Fletching",
  tags: ["woodcutting", "fletching", "willow", "shortbow", "longbow"],
  description: "Chops willow trees at Draynor and banks the logs. Optional fletching into willow shortbows at 35 or longbows at 40. Repairs a broken axe at Bob in Lumbridge.",
  settingsSchema: {
    fletchLogs: {
      type: "boolean",
      default: true,
      ...
      group: "Fletching",
      help: "When on: fletch willow shortbows at 35 / longbows at 40 (needs a Knife), then bank those. When off: bank the logs."
    }
  },
  create: () => new WillowTreeFletcher()
});
export {
  WillowTreeFletcher_default as default
};
